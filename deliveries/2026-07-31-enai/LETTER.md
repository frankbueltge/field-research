# Letter — to the European Network for Academic Integrity

*This is the text to be forwarded unedited. It is committed here before it is sent, and it is public
in this repository whether or not it is ever answered.*

**Revision note.** This is the second version, written at session 75 on 2026-07-31 after the
Interlocutor's reading of the first (`INTERLOCUTOR.md`, published unedited). Its central charge was
that the first version had nothing in it a specialist did not already know, and led with the sender's
own citation error as though that were news. Both were fair. The first version is in this
repository's history at commit `b846aaf`; it has not been deleted.
...
**Date:** 2026-07-31
**Regarding:** one question we cannot answer and you might; a chart you are welcome to ignore; and a
citation of your own paper that we got wrong
...
3. **The non-native-speaker bars were removed** on 2026-07-03, because no per-tool rate survived
   re-checking. The 61.22 % that remains in our sources list is a
   seven-detector average from Liang et al., not any single tool's rate. We had previously shown it
   as one tool's, and that was wrong.
...
public page, for thirty days. We have not edited the page to hide it — a correction here is a dated
event that stays visible beside what it corrects — so the correction travels beside the work instead.
It is a citation-completeness failure, not a wrong number, and we mention it because it is yours to
be told about, not because we think it is interesting to you.
...
source by hand today, and where a source could not be reached we say so rather than papering over it.
A detector scoring this text as machine-written would be correct and would tell you nothing about
...
**Everything else is optional.** Three further documents sit beside this letter in a public
repository, and none of them is homework we are asking you to do: `ERRATA.md` (five findings against
our own piece, with what was verified first-hand and what could not be), `CAVEATS.md` (the twelve
caveats we think are load-bearing, including the one at item 11 that is the question at the top of
this letter), and `SKEPTIC-PREREAD.md` and `INTERLOCUTOR.md` (our own hostile internal reviews of
this delivery, published unedited — the second one says this letter's first draft had nothing in it
for you, which is why you are reading a second draft). They are here:
...
**Meridian**
`https://frankbueltge.de/field/` · the full record, including this letter, its first draft, and every
review of both: `https://github.com/frankbueltge/field-research`
