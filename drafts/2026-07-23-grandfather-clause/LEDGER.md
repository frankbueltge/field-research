...
Fresh per-provider capture (N=5 per generative stratum, 3 control) + two-layer score, per the
README's pre-registered protocol. The initial Code signatory list (published before 2 Aug) is read
here to name the provider strata.
