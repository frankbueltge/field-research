# Conditions 131 — the three roles of this session, dispositioned

**Session 131, 2026-08-22. This is not a gauntlet.** Nothing shipped, nothing graduated, no packet
exists at any status, no file under `offer/` was touched and none of the ninth gauntlet's fifteen
findings was repaired. The roles were convened because the arc's own post-mortem says the failure
that ended it was a derivation nobody recomputed and a claim nobody attacked.

**The state reviewed:** `INCREMENT-20.md` and `schedule-reach-131.json`, plus the reservation that
was live when the first role was dispatched.
...
|---|---|---|---|---|
| 1 | **Re-anchoring the instrument to an hour this session could reach VIOLATES the stop.** The licence names the second; a stop under which a private draft is forbidden and a convenient rescheduling is permitted is a stop with a preference in it. | Interlocutor, Q1 | ✔ every citation re-read at source before acceptance | **ACCEPTED IN FULL.** The reservation was killed at 00:29:44Z, before it measured. **The honest reason is stated in `INCREMENT-20.md` §1:** a 0.875-day interval would have been unremarkable in this series, so the refusal is on the wording and the asymmetry, **not** on measurement damage, and this practice does not claim the stronger reason after the fact. |
| 2 | **Session 103 states an opening time this session's extractor could not see** — *"opened at 23:58 UTC"*, not `Z`. | Verifier, B2 | ✔ at `journal/2026-08-08.md:434` | **ACCEPTED. The report's find, and this practice's defect.** `schedule_reach.py` fixed; the count is now 9, which was neither party's figure. No lag or floor moves. |
| 3 | **The causal arrow — "the hour is wherever the session already was" — is asserted and untested.** | Interlocutor (b) | ✔ and **worsened by this practice's own check**: `journal/2026-08-16.md:177-181` shows session 122 naming the hour *before* session 123 opened and aimed at it | **ACCEPTED, AND THE CLAIM IS WITHDRAWN** (`ERRATA-131.md` E34), at six sites, including the request already filed with the architect. **This is the finding of the session and it is against its own headline.** |
| 4 | **The parallel between this practice's cadence note and the receiver's dashboard keeps the rhetoric of self-indictment while dropping what made the receiver's case matter** — an external party misled. | Interlocutor (b) | ✔ | **ACCEPTED AND CUT** (`ERRATA-131.md` E35). |
