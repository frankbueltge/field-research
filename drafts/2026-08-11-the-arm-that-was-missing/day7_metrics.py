#!/usr/bin/env python3
"""day7_metrics - the day-7 interval, computed to a file rather than typed into prose.

Session 125, 2026-08-17. Same shape as `day6_metrics.py`, which it is adapted from unchanged in
method. Day 7 is the SEVENTH consecutive daily run and completes the pre-registered window.
The day-6 side uses the canonical run file, not the second probe of that date.
Every figure the journal entry and the day-7 record quote comes from here.
"""
import calendar
import json
import time

D5 = "ledger/run-2026-08-16T0337Z.json"      # the previous day: day 6, canonical run
D6 = "ledger/run-2026-08-17T0337Z.json"      # this day: day 7
DIFF = "ledger/diff-day6-day7.json"
CONFIRM = "ledger/transition-confirm-2026-08-17.json"
...
out = {
    ...
    "computed_by": "day7_metrics.py, session 125",
    "interval": 6,
    "window_position": "seventh consecutive daily run; completes the pre-registered window",
    "run": {
        "file": D6,
    ...
    "apparent_transitions_overlay": json.load(
        open("ledger/diff-day6-day7-overlay.json"))["n_transitions"],
    "overlay_rows_applied": json.load(
        open("ledger/diff-day6-day7-overlay.json"))["corrections_applied"]["n"],
    "k4": conf["K4"],
    ...
    "denominators_this_interval": {
        "retrievable_at_day6_and_determinate_at_day7": len(ret_at_5_det_at_6),
        "absent_at_day6_and_determinate_at_day7": len(abs_at_5_det_at_6),
    },
    ...
    "per_arm_counts_day7": d6.get("counts"),
}
json.dump(out, open("day7-metrics.json", "w"), indent=1)
