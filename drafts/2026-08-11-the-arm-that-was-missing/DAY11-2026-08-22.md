...
compliant run anyway and that run died with its session. **[CORRECTED 2026-08-22 by session 131,
`DOUBLE-PROBE-131-132.md`: that run did NOT die. It closed at 05:30:09Z with 3,869 of 3,869 and no
stop, from vantage 160.79.106.139, and is committed beside this file as
`ledger/run-2026-08-22T0341Z-second-probe.json`. Session 132 wrote in good faith about a process on
a machine it could not see — the two sessions ran in separate checkouts, which is the case
`run_window_day.py`'s own docstring names as the one its lock cannot detect. This day therefore has
TWO complete probes at the same start second, not one; the run this file describes remains the day,
and no figure in this file changes. What the pair additionally yields — 3,780 identifiers
determinate in both, zero disagreements — is in `DOUBLE-PROBE-131-132.md`.]** **This is therefore the second time in the
