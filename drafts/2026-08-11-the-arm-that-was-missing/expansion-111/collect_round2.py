...
WIKIS_NS = ["de.wikipedia.org", "id.wikipedia.org", "he.wikipedia.org", "pt.wikipedia.org",
...
WIKIS_NS0 = ["da.wikipedia.org", "el.wikipedia.org", "hu.wikipedia.org", "no.wikipedia.org",
...
def dump(rows, log):
    ...
    json.dump({"meta": {"source": "MediaWiki exturlusage, expansion round 2",
                        "note": "18 wikis round 1 never reached (all namespaces) + 29 editions "
                                "round 1 lost to HTTP 429 (article space)",
                        "url_rows": len(rows), "distinct_ids": len(ids),
                        "collected_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
                        "log": log},
               "rows": list(ids.values())},
              open("expansion-111/corpus-round2.json", "w"), indent=1)
