...
def main(deadline_utc_hhmm, out="expansion-111/manifest-baseline3-111.json"):
    already = {str(o["vid"]) for o in
               json.load(open("ledger/run-2026-08-11T1124Z.json"))["observations"]}
    for prior in ("expansion-111/baseline-run.json", "expansion-111/baseline-run2.json"):
        if os.path.exists(prior):
            already |= {str(o["vid"]) for o in json.load(open(prior))["observations"]}

    d = json.load(open("expansion-111/corpus-round3.json"))
    units = {}
    for r in d["rows"]:
        v = str(r["vid"])
        if v not in already:
    ...
    man = {
        ...
        "purpose": "session 111 THIRD baseline — the round-3 identifiers, before day 2 opens",
        "seed": SEED,
        "cap": {"deadline_utc": f"{deadline_utc_hhmm[:2]}:{deadline_utc_hhmm[2:]}Z",
                ...
                "note": "dropped identifiers remain in expansion-111/corpus-round3.json and "
                        "may open a separately dated arm in a later session; they cannot "
                        "carry this window's intervals"},
        "arms": {"round3": {k: v for k, v in d["meta"].items() if k != "log"}},
        "units": kept,
    }
