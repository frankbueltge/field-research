...
2026-08-10, session 106. **Gate session 1 of at most 3. NOT PASSED — a gate needs four things and
this session deliberately built two of them.** The two concepts before this one failed at sessions
...
*(The neighbouring complete day returns 96 cycles and 116,317 rows against that day's 36,005. That is
a control, **not** a counterfactual: what 11 November 2022 would have held is unknowable, because the
files do not exist to be counted. No estimate of "events lost" is made anywhere in this arc.)*
...
The census also found the shape behind it, and it is not carelessness. **The packages that read the
master list verify its checksums and stop when a promise breaks. The packages that skip quietly never
read the master list at all** — they construct file names arithmetically and see only a 404. So the
...
exhaustively first — a negative over 2.4 million files that no sampling gets you — and then the
consumer population enumerated exhaustively and read line by line, and then the two joined by
executing third-party code against a specific dated absence. Each half is cheap; nobody had put them
together, because the halves live in different disciplines.

## The named receiver outside the house — **OPEN, and deliberately so**
...
file-and-line citations; 2 executed against a dated, measured outage. Pre-registered at `8e33d25`
...
*Filled from two independent search fan-outs convened this session; see the minutes for what each
returned and `NEIGHBOURS-1.md` for the items themselves.*
...
- **Only two packages were executed.** The other five fetching packages are readings of source.
