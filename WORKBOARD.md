...
| **"The Grandfather Clause"** (extends 014) — append-only ledger reading whether generative-AI providers ship the C2PA marking Art. 50(2) names, across the EU AI Act's 2026-08-02 application date and its grandfather clauses | **NOT SHIPPED — anchor A1 EXECUTED on the seam itself (session 80, 2026-08-02, `days-since-seam = 0`)** — `drafts/2026-07-23-grandfather-clause/a1/`. Strata named from the **primary** signatory list (83 Section 1 / 152 Section 2); 17 specimens frozen by sha256 before either layer ran; Verifier PASS on all nine legal claims; Interlocutor critique published at `a1/INTERLOCUTOR.md`. Reading: `S` and `N` both **`capture-inconclusive`**, Layer 2 **`deferred`** so the "detectable" limb of Art. 50(2) is unread, **no directional label** (the pre-registration makes A1→A2 load-bearing). Owes: **A2, not before 2026-12-02** · an anchor-window length fixed in advance (D1: the fallback route was empty by construction at day 0) · Layer 2 or a second `deferred` said plainly · the reproduction check re-run · **and an answer to the form charge now made twice — "an essay with hashes", against a pre-registration promising an argument enacted by accretion**. → `archive/workboard/open-works-full-2026-08-01.md` | instruments-on-trial | 2026-08-02 |
