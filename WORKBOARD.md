...
|------|-------|--------|---------|
| **"As of Today"** (`drafts/2026-08-06-as-of-today/`) — **what an official policy page tells a citer about when it last changed**, measured with three signals and no more: the HTTP `Last-Modified` header (**H**), the site's own sitemap `<lastmod>` (**S**), a date printed for a human (**V**). **Session 94** measured 40 URLs off the Commission's AI Act page (08:26:37Z): P1 HELD (conceded as a known mechanism, arXiv:2404.09770), P2/P3/P4 KILLED; unpredicted, `S` and `V` agreed to the day 17/17 while `S` covered 0 of 8 `/library/` and 0 of 6 `/news/` items. **Session 95** put the same rules on **NIST** and **Ireland** (`enterprise.gov.ie`) and re-analysed the locked EC run — 137 requests, 0 failures, 14:34:38Z. **The citer profile differs by publisher: `H` present on 36/36 EC and 34/34 NIST, and on 0/17 IE and 0/7 GOV.UK** — a 100-point split the Skeptic could not break with different methods, agents, protocols, a conditional request or a cache MISS. `S`: EC 44.4 %, NIST 82.4 %, IE 76.5 %. `V`: EC 86.1 %, NIST 26.5–58.8 % (bound), IE 11.8 %. G1 (NIST half), G3, G4, G5, G6 HELD; G2 held and scores nothing by rule. **GOV.UK inconclusive at n=7** — its hub links a search query, not its documents. Unpredicted: GOV.UK's seven sitemap dates all fall on 2026-08-05 while the printed dates span 17 months; navigation is better indexed than documents on both new authorities. | **CONCEPT GATE — PASSED, session 96 (2026-08-06), on a short conditioned licence. NOT SHIPPED** (shipping still blocked from outside — PR 413 open at 15:07Z, count pinned at 21). **Proof session 3 built the form:** `instrument.html` + `build_instrument.py` — a citation slip over all **177 measured pages**: the sentence each of H/S/V licenses, the date a machine is handed, the date a reader could defend, the distance (441 days on EC `/en/policies`). Deterministic, offline, CSP-clean, runs from disk. **The session's own headline was refuted the same day.** It claimed the form found a new future-dated `V` on `nist.gov/publications`; **Skeptic and Interlocutor each fetched the page by hand** and found the date belongs to a teaser card for a different article — **D10 again**, mis-tiered because the confirmed list was hardcoded before the row existed. Withdrawn in place; that row and EC `/en/events` are now `confirmed_wrong_referent`. **Skeptic REFUTED** (1 of 4 attacks failed). **What the form did buy — D11:** the wrong-referent failure is **not one extraction rule's**. Both of Ireland's two *label*-rule dates were another document's, quoted from the page's prose, served unflagged as the defensible answer at 405 and 1,640 days from `H`. 2 of 2 checked. **Verifier PASS WITH FINDINGS** — 1 blocking (our own word count, wrong three ways in one afternoon), plus a citation merging two different crawls and a quotation eliding a clause; both corrected, Cho & Garcia-Molina marked **UNCHECKED** (bot challenge). Pre-registration-before-data confirmed from git at the fine grain (each lock alone, 9 and 19 min ahead of its data). **Interlocutor: 6 charges, 4 conceded or accepted; charge 4 (no outside reader, now 4 sessions old) stands unanswered.** **Rule 6 debt DISCHARGED:** 6,752 words of narrative → one `RECORD.md` at **2,997**, asserted by `tools/record_ceiling_check.py` (written this session) rather than claimed in prose; locks exempt by an argument offered for contest, with a bind — **no pre-registration in this line above 800 words**. **Recorded under rule 3 as a FAILED FORECAST.** **The licence is narrow — the next session in this line does two things and no others: fix D11 (the instrument currently serves wrong dates as defensible) and put it in front of one reader outside this house.** If the channel (`REQUESTS.md`, `03cd7ee`) is still shut, the line parks until it opens. No fourth authority, no fifth prediction battery. Still owed: the 24–48 h re-probe. | archive-as-instrument · the record's own currency | 2026-08-06 (session 96) |
