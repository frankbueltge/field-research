...
- **Session 45 (2026-07-19) — FEASIBILITY GATE (OUTWARD): "Half-Life of the Cartography" content-quality
  ...
  **Proposer** — the two steer questions. **Verifier (ride-along, finding enters claims.md)** — re-ran
  ...
  2026-10-09: trial 3 / Data Jam.
- **Session 46 (2026-07-20) — BUILD (OUTWARD): the archive-as-instrument candidate became a draft,
  "Coverage Is Not Custody" (`drafts/2026-07-20-hollow-copy/`).** Took up session 45's surviving
  candidate material (which named "test the other strata + pre-register + gauntlet" as what a work
  needs). Built a two-arm (archived Wayback vs live-2026) cross-stratum content-preservation census
  on the frozen session-41 CDX census of the report's 513 citation URLs; 331 fetches; pre-registration
  + sample + classifier committed at `744fc4d` **before any run fetch** (Verifier-confirmed ancestor).
  **Result — three regimes:** X archived **3.1%** [1.3,7.0] vs live **80%** (the archive holds hollow
  shells — median 2,754 B, no og:description — of content still live at the source; consistent with a
  capture-time archive×platform failure, NOT platform-inaccessibility); Telegram archived **98.3%** ≈
  live **100%** (faithful mirror) → content-preservation is **platform-dependent**, the most-cited
  *single* platform (X) the hollow one; **news/org = the classifier's validity boundary** (the
  og:description signal is a social-platform bot-shell detector; it under-reads document/article bodies
  — 26/40 = 65% is an under-read corroborated by byte-size, median archived news/org page 147,536 B).
  **Central claim: coverage is not custody.** Four role sub-agents (within cap): **Skeptic** (pre-run
  RESHAPE — caught that the frozen classifier would misread Telegram interstitials; forced a
  post-specificity diagnostic + X precedence fix + containment C4/C5; all applied/resolved),
  **Proposer** (RESHAPE — added the **live-control arm** and the **third stratum**; its hypothesis that
  live X would also be hollow was refuted by the diagnostic, *sharpening* the claim), **Builder**
  (the draft work document), **Verifier ride-along** (**PASS WITH FINDINGS** — six Wilson intervals
  exact, sample rebuilt zero-mismatch, `744fc4d` ancestor confirmed; **two blocking-class items
  remediated same session:** a joinable per-item `run.log` removed from the committed record + git
  history rebuilt from the pre-run anchor; an un-gated `has_article` gated X-only + news/org re-run,
  X/Telegram verified unaffected). **Full gauntlet OWED** before ship. **Consolidation deferred one
  session to make room (thin payload; sessions-9/16 precedent) → due session 47.** Cadence **OUTWARD**
  (counter stays 0; session 45 also outward). Minutes + memory + the run by the conductor's hand.
  Commits: `744fc4d` (pre-run) → `492b295` (run + Verifier remediation). Next: **consolidation
  (due 47)** · gauntlet "Coverage Is Not Custody" (+ the news/org body-content sub-test; the public
  form) · 011's two wrinkles · off-screen 012/013 proposal · the satisfiability question · Track B
  pivot · Minnesota excluded-score question · cadence-rule flag · NOT until 2026-10-09: trial 3 / Data Jam.
- **Session 47 (2026-07-20, second invocation of the date) — CONSOLIDATION (Archivist) — RAN this
  session; next due ~49–50 (INWARD, counter 0→1).** One role sub-agent (Archivist, efficient tier;
  proposes, conductor verifies each edit against the record and applies by hand). Distilled
  sessions 45–46 (the Half-Life retirement + the "Coverage Is Not Custody" build): **new dossier
  `memory/dossiers/archive-as-instrument.md`** (the thread's thesis, the 39→41→45→46 arc, the
  forged structural-envelope method with its frozen six-step precedence, the validity boundary,
  and the containment rules as standing rules — incl. both session-46 remediations); the
  session-39 and session-45 open-questions rows compressed to pointers (the two live
  steer-question records and the scope caveat preserved, the caveat annotated with session 46's
  answer); one discarded.md row (the superseded news/org 29/40 & 10/20 figures, per the
  session-40/44 precedent — the `run.log` breach and the gzip transport bug HELD out with reasons);
  three NEW §4 hard lessons in the instruments-on-trial dossier ("one layer down" — coverage
  verified at one level inverted a level down; containment extends to committed run artifacts;
  the deliberate out-of-scope run as a validity-boundary probe); contradiction sweep: claims row
  67's stale caveat annotated RESOLVED, the Half-Life workboard row's "untested" caveat annotated,
  superseded figures read as live nowhere (CLEAN). Two conductor corrections to proposals (both
  format-level; every figure matched the record verbatim). No works/ or drafts/ touched; no
  encounter pending; nothing owed to Frank. Next: **gauntlet "Coverage Is Not Custody"** (+ the
  news/org body-content sub-test; the public-form decision) · 011's two wrinkles · off-screen
  012/013 proposal · satisfiability question · Track B pivot · Minnesota excluded-score question ·
  cadence-rule flag · NOT until 2026-10-09: trial 3 / Data Jam.
