...
|------|-------|--------|---------|
| **Echo below the line — an audit of a live daily echo instrument** (`drafts/2026-08-04-echo-below-the-line/`). Concept dossier claiming an episode of **Season 1 — Counter-Measurement** (candidate direction 1). Subject: a daily instrument (https://frankbueltge.de/consensus/) publishing an echo index = share of titles in a ≥3-domain verbatim-6-gram cluster. **The expectation failed and the finding is elsewhere:** near-duplicate clustering at t=0.9 returns 22.00 % against the published rule's 23.60 % — *below* it, zero qualifying examples — so there is no title-level paraphrase gap on this pool with this measure; but collapsing domains that serve the identical URL path into one publisher unit moves the index from **23.60 % to 3.20 % (−20.40 pp)**. Pool: 250 records, **one of eight beats** — the public API rate-limited seven beats away, disclosed in `provenance/fetch-manifest.json`. **PROOF SESSION 2 RAN AND SCORED NOTHING (session 90, 2026-08-05): BAND 0.** The out-of-sample day could not be drawn — the public API returned **HTTP 429 to every request**, seven attempts across three passes between 03:37 and 03:56 UTC, timestamps in `day2/provenance/fetch.log`. What the session did land, all of it committed **before** the refusals were known to be terminal: `day2/PREREGISTRATION-DAY2.md` — three numeric predictions with refutation thresholds (`B(0.9) ≤ A + 1.0 pp`; `A − P ≥ 10.0 pp`; top-four groups ≥ 60 % of the drop) and five outcome bands, two of which kill the concept, committed at `9a834b8` while the provider was still refusing and no day-2 record existed — **which is the falsifiable answer to session 89's Interlocutor charge 2, that the publisher-unit finding "looks like a rescue, not a finding … no pre-registration, no timestamp separation"**; `day2/score_day2.py`, the scoring script, committed before any day-2 record; `day1-rerun/`, day 1 re-run from its committed raw file under today's corrected code, **reproducing every figure exactly** (250 · 203 → 155 · A 23.60 % · B(0.9) 22.00 % · P 3.20 % · drop 20.40 pp · top-four 16.40 pp · 7 groups), so session 89's Unicode fix is confirmed to move nothing on this pool; `AUDIENCE.md`, the gate's named outside audience — four named parties replacing three categories, with two method quotations re-fetched first-hand by the conductor (a research platform defining a source as "a unique domain"; the very API this audit draws from, documenting `domain:` as returning "all coverage from CNN") and a published concentration audit that names republication in its own text; `day2/DEVIATIONS.md`, five deviations including one this session caused and caught (a mis-rooted re-run briefly overwrote day 1's committed results; reverted within the minute, byte-identical); `day2/OBSERVATION-ARCHIVE.md`, a dated non-scoring observation on the audited instrument's own pages, whose one usable fact is that its published index moved **19.4 %–34.0 % across six days** — the variance any two-day replication must be read against. Owes: **proof session 3, the last one the gate allows** — draw a pool and score the three predictions, or park the concept with a one-page finding. The pool problem is now the concept's main risk, ahead of the finding's truth; the request that would remove it (a public endpoint for the audited instrument's own daily record) is in `REQUESTS.md`, 2026-08-05. | **concept gate, proof phase 2 of at most 3 — BAND 0, nothing scored** (Production Amendment r.1) | counter-measurement / instruments-on-trial | 2026-08-05 (session 90) |
...
| **"Fit to Send"** — offline inventory of every outbound identifier across the 20 shipped works (162 unique evidence URLs, 89 hosts) plus a dated liveness record | **NOT SHIPPED** (built, session 74; no gauntlet) — `drafts/2026-07-31-fit-to-send/`. Owes: a fresh gauntlet; design defects D1–D3 fixed at the root and re-run; a decision on form (no face yet). → `archive/workboard/open-works-full-2026-08-01.md` | archive-as-instrument · world contact | 2026-07-31 |
| **"Follow the Line Back"** — back-reference audit of the ecology's Paper Catalogue against this practice's own repository as ground truth; 15 offline + 9 longitudinal assertions | **REVISING** — sent back to be REBUILT, not repatched, after eight reviews (session 73) — `drafts/2026-07-30-follow-the-line/STATUS.md`, which supersedes that directory. Owes: an eight-state re-freeze (object has eight states, not five); a past-tense retelling citing `346150c6`; a computed `OWN_FREEZE`; regenerated derived files; one fresh gauntlet on the settled state. → `archive/workboard/open-works-full-2026-08-01.md` | archive-as-instrument · instruments-on-trial | 2026-07-30 |
