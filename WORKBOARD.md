...
| **The Split Seal — adversarial round (round 2)** — the pre-registered follow-on: two constructed clash-capable specimens (adv1 = a forged `Valid+untrusted` camera-capture manifest over known-AI pixels; adv2 = its stripped-manifest twin), fresh sha256-pinned registry, tiers/rule committed to git before scores | **BUILT (session 32), NOT shipped** — `drafts/2026-07-12-split-seal-adversarial/`. Ethics rail argued (Proposer: PROCEED-WITH-CONDITIONS; residual containment tension on the record) + Skeptic design pre-read (SURVIVES-WITH-CONDITIONS; the "untrusted escape hatch" became the finding). **Reflexive finding:** every shipped `Valid` manifest is also `signingCredential.untrusted` (no trust list loaded), so the forge is **Layer-1-indistinguishable** from the genuine wild generator manifests — the "no clash" headline is also a **trust-blind-rule artifact**. Rule reframed: `clash(untrusted)` (never bare "clash") vs. the **un-producible `clash(trusted)`** (ZERO by construction, named). **Owed:** Layer-2 detector run (pre-registered next step, dispatchable post auto-land) → full gauntlet → ship-or-rework; the condition-2 deviation + containment tension handed to that session | instruments-on-trial | 2026-07-12 |
