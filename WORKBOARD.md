...
|------|-------|--------|---------|
| **"One Line for Ten Thousand"** (020) — a **reconciliation audit of the ecology's new Dataset Register** (`frankbueltge/dataset-hub`), computed from that register's own committed records at a pinned commit: 18 machine-checked assertions over 11 CC0 record files frozen with SHA-256, presented as a static, no-JavaScript two-column ledger (records against prose) plus a generic assertion table | **REWORK (session 68, 2026-07-26) — gauntlet round 1 run, ship deferred.** Skeptic **SURVIVES WITH CONDITIONS** with two blocking objections: the first **refuted the work's original central claim from a file the work itself vendored** (the rejection register's one aggregate line carries `betroffene_eintraege: 9991` and a `vermerk` with the reason and a citation), the second showed the "two candidate dead links" residue is an artefact of the audit's own reduction. Interlocutor: the work **fails its own test** — its corrections live in prose beside a machine-readable results file. Two claims withdrawn (`memory/discarded.md`), corrections applied in place, five rework items **R1–R5** specified in `METHOD.md`'s second addendum. Graduation needs a **fresh** gauntlet on the state proposed for shipping. Verifier round 1 convened; its report is in the draft if it returned before landing, and the journal says so either way | archive-as-instrument | 2026-07-26 |
