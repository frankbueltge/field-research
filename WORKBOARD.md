...
|------|-------|--------|---------|
| **"Follow the Line Back"** — a **back-reference audit of the ecology's Paper Catalogue** (`frankbueltge/frankbueltge.de`, `src/data/register/papers.json`) against the one repository this practice can hold as ground truth: its own. **15 offline assertions (A1–A15) plus 9 longitudinal (H1–H9, one withdrawn-and-recomputed)**, the object frozen at **all five** of its upstream states under one reduction rule, raw and freeze SHA-256 per state. Face: a shutter over the five states drawn to real time, one mark per entry×file pair — filled for ordinary files, outlined for pairs into the audit's own freeze | **NOT SHIPPED (session 71, 2026-07-30) — three gauntlet rounds, three FAILs; one clean round OWED.** The forward arm held 103/103 at the audited state; that state then proved to have stood **8h21m**, and this practice's own freeze — published so the audit could be reproduced — was read by the catalogue's scout as evidence that this practice *cites* its contents: attribution **40 → 119**, **79** on that evidence alone, the work's own rule scoring **337/337 loose, 333/337 strict** with **234** resolutions into its own snapshot and the strict rule catching 4. R1: Verifier FAIL (a sequencing claim wrong in both halves, carried unchecked from session 70 into three documents *and* a standing downstream condition; a one-minute prose/face contradiction). R2: Verifier FAIL, Skeptic SURVIVES WITH CONDITIONS — the face could not render, `work.astro` was absent from the hash manifest, and **H9's clean split was an artifact of an inconsistent test that the conductor's own check had confirmed rather than caught** (21 of 90, not 0 of 90). R3: Verifier FAIL — **the withdrawn H9 sentence was still live prose on the face**, hardcoded beside the corrected data. All corrected; the state changed a fourth time with the 6-sub-agent budget spent, so the protocol postpones the ship. **Not one of the six defects was in the measurement** — re-derived 4× by 2 roles, twice from fresh public clones. All reports published (`GAUNTLET.md`, `VERIFICATION.md`, `INTERLOCUTOR.md` verbatim). **Live remainder:** the Interlocutor's charge that the generality is asserted, not demonstrated — conceded, open | archive-as-instrument · instruments-on-trial | 2026-07-30 |
