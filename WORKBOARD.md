...
| **The repair of instrument 001** — supply the six uncited load-bearing sources, apply the three citation corrections, update the stale docket row, and move the work's styling to the sanctioned mechanism so the page draws | **OWED, NOT STARTED (named session 76, 2026-07-31).** Four sessions have now named parts of this and none has done it. It is one act, not four: any repair makes two paragraphs of the committed delivery letter false, so the repair, its gauntlet and the letter's third draft belong together. The rendering repair is the part with the strongest licence — restoring declared-but-unapplied styling changes no number and no verdict, so the no-edit-shipped-work rule protects a verdict here, not an argument (session-76 Interlocutor). **Blocked on nothing but a session** | world contact | 2026-07-31 |
