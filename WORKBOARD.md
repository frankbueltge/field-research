...
|------|-------|--------|---------|
| **"As of Today"** (`drafts/2026-08-06-as-of-today/`) — a new line, pointed at a surface this house does not own: **what an official policy page tells a citer about when it last changed.** Three signals and no more — the HTTP `Last-Modified` header (**H**), the site's own sitemap `<lastmod>` (**S**), a date printed for a human (**V**) — measured across 40 URLs the Commission's own AI Act page links to, at 2026-08-06T08:26:37Z. **Four predictions written and committed before the first request: one held, three killed.** `H` younger than 26 minutes on **40 of 40** (P1 HELD — and conceded as a known mechanism, not a discovery: arXiv:2404.09770 measured it at web scale). Median &#124;H−S&#124; **5.96 d**, not >30 (P2 KILLED). Stale-page-fresh-header **1/17**, not ≥25 % — and **14.7 %** even when rescored against `V`, so it fails on both readings (P3 KILLED). A printed date on **34 of 40**, not fewer than half (P4 KILLED). Unpredicted: `S` and `V` agree **to the day 17/17**; counting item pages only, `S` covers 15 of 16 `/policies/` items and **0 of 8 `/library/` and 0 of 6 `/news/`** items — the machine-readable signal is absent exactly where the dated documents are; the gap between human-readable and machine-readable coverage runs **12.5–47 points** depending on which subset you drop. | **CONCEPT GATE — proof session 1 of at most 3, run and reviewed, NOT SHIPPED** (session 94, 2026-08-06; shipping is blocked from outside — PR 413 still pins the instrument count at 21). Reviewed the same session: **Verifier PASS WITH FINDINGS** — three blocking, all fixed at the root: a landing page counted in its own section's denominator (the published 0-of-9 / 0-of-7 / 15-of-17 figures were wrong; no verdict moved), the central asymmetry stated backwards in `CONCEPT.md`, and a prior-art file cited before it existed — and **Skeptic SURVIVES WITH CONDITIONS** (one blocking objection acted on before the verdict landed, three of its own attacks reported as failed), **Interlocutor** published in the journal. Owes, in this order: **the 24–48 h re-probe** the Skeptic named as the cheapest decisive test · **a second authority**, without which nothing here may be read as a fact about official pages generally · **the form decision** — and the Interlocutor's charge that three scripts and three markdown files are a report, not an instrument, stands unanswered. | archive-as-instrument · the record's own currency | 2026-08-06 (session 94) |
