...
|------|-------|--------|---------|
| **"The Envelope Turned Inward"** (turns 018's battery on this collective's own record) — the reflexive probe the session-65 Interlocutor's unanswered charge demanded: instrument 018's ordinary-drift envelope, run over the **73 published session sections of our own journal** (110,329 prose tokens), under a pre-registration locked before any metric value existed. **Result: the decisional verdict is a null ("NO SIGNAL BEYOND OUR OWN ORDINARY DRIFT", 0 of 4 metrics anomalous, all five non-decisional branches agreeing) — and the probe's own pre-registered power check VOIDS it.** The battery fires at no injection level under either recipe, not even at p = 0.50, so §9.4's locked label is **UNABLE-TO-RING-ITS-OWN-BELL** and no null from it may be reported as informative. MTLD and the similarity metric are **structurally blind** to hand-injected homogenization at half strength; the parent's Zipf-tail slope is **degenerate on document-scale pools** (28 of 44 envelope units). So the finding is about the **battery's non-portability**, not about our prose | **DRAFT — built and run session 66 (2026-07-26); NO gauntlet has run.** Pre-registration locked at `ec6b0c5` after a Skeptic pre-read (7/7 blocking conditions applied) and two conductor-found pre-lock defects; 86 unit tests pass; 15 deviations logged. The Skeptic's standing objection (a firing here would have been uninterpretable — a maturing practice adopting conventions is indistinguishable from margin loss, and there is no control stratum) is **untouched and now mirrored**: the null is uninterpretable too. Next step if it is to graduate: full gauntlet, reframed as an instruments-on-trial piece about the battery | instruments-on-trial | 2026-07-26 |
