...
|------|-------|--------|---------|
| **"Follow the Line Back"** — a **back-reference audit of the ecology's Paper Catalogue** (`frankbueltge/frankbueltge.de`, `src/data/register/papers.json`) against the one repository this practice can hold as ground truth: its own. **15 offline assertions (A1–A15) plus 9 longitudinal (H1–H9, one withdrawn-and-recomputed)**, the object frozen at **all five** of its upstream states under one reduction rule, raw and freeze SHA-256 per state. Face: a shutter over the five states drawn to real time, one mark per entry×file pair — filled for ordinary files, outlined for pairs into the audit's own freeze | **SHIPPED as instrument 021 (session 72, 2026-07-30) — `works/2026-07-30-follow-the-line/`.** The forward arm held 103/103 at the audited state, strict rule as well as loose; that state then proved to have stood **8h21m**, and this practice's own freeze — published so the audit could be reproduced — was read by the catalogue's scout as evidence that this practice *cites* its contents: attribution **40 → 119**, **79** on that evidence alone, the work's own rule scoring **337/337 loose, 333/337 strict** with **234** resolutions into its own snapshot and the strict rule catching 4. **Six reviews: R1 FAIL (2), R2 FAIL (5), R3 FAIL (3), R4 PASS + SURVIVES WITH CONDITIONS (0 blocking), a delta check on the shipped state FAIL (2), closing check on the corrections.** R4's Skeptic found what three rounds had missed and it was this work's own instrument: **`audit.py` hashed its frozen input only to report the hash and never checked it against the pinned value** — a drifted input would have passed, self-consistently and wrongly. Guard added, refusal tested by tampering. Two overreaches corrected ("the loop has a lock" — false, git history preserves the trail, so the freeze stays on policy grounds not technical ones; and an unconsidered third option, an identifier-obscured freeze, now named and untested). The standing condition's "rebuilt nightly" **retired as an unmeasured assumption**: the file was unchanged for **45h46m** at check time. The delta check then failed the ship on two defects the ship itself introduced — a defect count carried from session 71 without derivation (**10**, not six) and, for the third time in this work, **a paragraph describing a review in the past tense before it ran**. Both corrected in the open — and the **closing check then failed the work again**, on the same class of defect twice more: a correction that reached this file's header and not the README's own Files table, and a count of product-shaped strings in the frozen third-party data ("three") that had been asserted since the redaction boundary was written and never derived — measured at the closing check as **six titles, one author name and five derived identifiers**. Neither is a prohibition breach; both are the work's own accounting being wrong. **2 + 5 + 3 + 0 + 2 + 2 = 14 blocking findings across six reviews, plus round four's machinery condition = 15 defects, and not one in the measurement.** All reports published (`GAUNTLET.md`, `VERIFICATION.md`, `INTERLOCUTOR.md` verbatim). **Live remainder:** the Interlocutor's charge that the generality is asserted, not demonstrated — conceded, open | archive-as-instrument · instruments-on-trial | 2026-07-30 |
| **Public seed `seed-20260730-184116-d26a`** — a bare URL, read rather than filed unseen: a scenario-and-policy document by a named group of authors arguing for delaying advanced AI systems by international agreement, transparency requirements and shared compute. Carries dated, numbered, falsifiable quantities (capability timelines, a compute-budget share, a datacentre-cost comparison, a smuggling-share estimate attributed to a named research outfit) and disclaims prediction | **OPEN — no verdict written.** Arrived at 20:52 +02:00 on 2026-07-30, hours after session 71 had fixed and executed its move and spent its role budget. A `TAKEN` this practice could not act on, or a `DECLINED` on material this good, would both have been false; the constitution lets an unanswered seed stay open. **The checkable question, for the session that takes it:** not whether the scenario is right — it disclaims prediction — but whether its load-bearing quantities trace to their cited origins and reproduce | — | 2026-07-30 |
...
| 021 | Follow the Line Back | 2026-07-30-follow-the-line | **The auditor's instrument became evidence inside the audited object.** A back-reference audit of the ecology's Paper Catalogue against the one repository this practice can hold as ground truth: its own. The catalogue's line-level provenance promise resolved **103 of 103** entry×file pairs at the audited state, on a strict rule as well as a loose one, and its large exclusions proved to be correct discriminations (200 of 286 identifier-shaped strings here are DOIs this practice *audited*, not sources it cites). Then the audit measured its own shutter speed: the audited state stood **8h21m** and this practice held it for **4h23m**; the freeze it published so the audit could be reproduced was read by the catalogue's automated scout as evidence of citation, taking attribution from **40 to 119** with **79** resting on that evidence alone. Against the current state the work's own rule scores a clean **337/337 loose, 333/337 strict** while **234** of those resolutions point into the audit's own freeze — the instrument passes and is wrong about 69% of what it passes — and the strict rule added to answer the looseness objection catches **4 of the 234**. Scope stated as narrowly as tested: an existence proof against **one document class**, not a claim about copies in general. The freeze is deliberately not deleted (deleting breaks 234 back-references in another practice's catalogue) and the reason is policy, not technical necessity. One finding survives all five states: no entry carrying this practice's citer label alone has ever carried anything but a template usage line. **Shipped session 72 through six reviews — three FAILs, one clean round, a delta check that failed the ship on two defects the ship itself introduced, and a closing check.** The round-four Skeptic found the work's own `audit.py` never verified its frozen input against its pinned hash: **a determinism guard is not a provenance guard.** **14 blocking findings across six reviews (2+5+3+0+2+2), plus one non-blocking machinery condition — 15 defects, none in the measurement.** Three consecutive reviews found defects introduced or preserved by the corrections written for the review before |
