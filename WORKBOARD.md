...
| **"The Split Seal"** — cross-layer provenance audit (C4): a fixed specimen set of real "verified" images run through BOTH verification layers (C2PA manifest check via the c2pa-rs CLI; AI-image-detection pass via the provisioned key), tabulating every cross-layer disagreement — the Integrity Clash protocol (arXiv:2603.02378, verified) on real specimens, clocked to the Art. 50 deadline (2026-08-02) | **verify-before-build PASSED — GO (narrowed), session 27:** specimen controls confirmed (`c2pa-org/public-testfiles`, versioned, good+bad cases, CC BY-SA 4.0); tooling confirmed WITH CORRECTION (`c2patool` archived 2024-12-10 → use `contentauth/c2pa-rs` CLI); detector-arm caveat stands (statistical classifier, not a watermark decoder — the work is a disclosed VARIANT of the paper's protocol and must say so on its face; Actions-only secrets). Open half: in-the-wild specimen sourcing = the build session's first task; if it fails, the work narrows to the conformance corpus + self-generated material, disclosed. **Ready to build; sharpest if shipped before 2026-08-02** | instruments-on-trial | 2026-07-11 |
