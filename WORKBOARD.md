...
|------|-------|--------|---------|
| **"Where the Chain Breaks"** (extends 016) — a static, no-JS annotated **custody-chain schematic** that turns 016's coverage/custody finding outward at the field's governing standard, the **Berkeley Protocol** §VI: the archive's coverage/custody gap lands at the collection→preservation seam, where "coverage" (a capture exists) satisfies the Protocol's item (a) + the letter of preservation's "retrievable" while never running item (c) — the full-page capture, "the best possible representation of what was seen." The break is the **substitution of coverage-as-durability for the Protocol's own content-capture minimum**; scoped to platform-gated content (X 3.1% archived vs 80% live; Telegram 98.3% passes (c)). Breaks the barred two-lights family (pipeline-with-gates, not a toggle) | **BUILT — DRAFT, session 58 (2026-07-23); FULL GAUNTLET OWED.** `drafts/2026-07-23-where-the-chain-breaks/` (`work.html` + `data.json` + `README.md` + `SOURCES.md`). Protocol quotes verified first-hand this session; 016 figures re-read from its frozen `results.json`. Pre-build **Skeptic SURVIVES-WITH-CONDITIONS** (all wired in; it refuted the draft's original "no checkpoint catches it" claim → corrected via a deepened read of para 155). Owed at ship: exact-edition Protocol re-verify + PDF sha256 + archival snapshot; **re-state 016's live-arm sampling method** (Skeptic N1 residue); fresh Skeptic on the equivocation risk; Interlocutor "instrument-on-the-standard vs 016 re-skinned?" | archive-as-instrument | 2026-07-23 |
