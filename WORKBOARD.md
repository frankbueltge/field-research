...
| **"The Split Seal"** — dual-seal cross-layer provenance register (C4): 15 frozen, sha256-pinned specimens (5 wild incl. 3 in-the-wild generator manifests found via Commons originals; 3 corpus camera captures; 7 quarantined synthetic fixtures) stamped by BOTH layers — C2PA manifest verdict (c2pa-python, local, deterministic) and raw detector score (Actions workflow, one pass) — verdicts computed from committed data; disclosed variant of arXiv:2603.02378 | **draft BUILT (session 28) — full gauntlet OWED.** Pre-registration order provable from git history (`ec84146` tiers-before-scores → `902332d` raw scores). Result on this set: **no pre-registered CLASH; both seals fire only where the producer volunteered disclosure** — the one community-labelled non-disclosing AI specimen is invisible to both layers (anecdote, not rate; so framed). Skeptic pre-read: BUILD-WORTHY, 7 conditions, all applied (README maps them). Sharpest if gauntleted + shipped before 2026-08-02 (Art. 50) | instruments-on-trial | 2026-07-11 |
