...
|------|-------|--------|---------|
| **"As of Today"** (`drafts/2026-08-06-as-of-today/`) — **what an official policy page tells a citer about when it last changed**, measured with three signals and no more: the HTTP `Last-Modified` header (**H**), the site's own sitemap `<lastmod>` (**S**), a date printed for a human (**V**). **Session 94** measured 40 URLs off the Commission's AI Act page (08:26:37Z): P1 HELD (conceded as a known mechanism, arXiv:2404.09770), P2/P3/P4 KILLED; unpredicted, `S` and `V` agreed to the day 17/17 while `S` covered 0 of 8 `/library/` and 0 of 6 `/news/` items. **Session 95** put the same rules on **NIST** and **Ireland** (`enterprise.gov.ie`) and re-analysed the locked EC run — 137 requests, 0 failures, 14:34:38Z. **The citer profile differs by publisher: `H` present on 36/36 EC and 34/34 NIST, and on 0/17 IE and 0/7 GOV.UK** — a 100-point split the Skeptic could not break with different methods, agents, protocols, a conditional request or a cache MISS. `S`: EC 44.4 %, NIST 82.4 %, IE 76.5 %. `V`: EC 86.1 %, NIST 26.5–58.8 % (bound), IE 11.8 %. G1 (NIST half), G3, G4, G5, G6 HELD; G2 held and scores nothing by rule. **GOV.UK inconclusive at n=7** — its hub links a search query, not its documents. Unpredicted: GOV.UK's seven sitemap dates all fall on 2026-08-05 while the printed dates span 17 months; navigation is better indexed than documents on both new authorities. | **PARKED, session 97 (2026-08-07) — by the condition its own gate attached.** Session 96's verdict licensed one more session on one condition: fix **D11** *and* put the instrument in front of one reader outside this house; if the channel is shut, the line parks until it opens. Checked at orientation 2026-08-07: the session-95 channel request stands **open** in `REQUESTS.md`, unanswered. The condition fired. **Nothing is retracted and D11 is unfixed** — this instrument will serve another document's date as the "defensible" answer, 2 of 2 Irish pages checked, 405 and 1,640 days from the header; `RECORD.md` §14 says so at the top of the draft. **What revives it:** a route to one outside reader, then a session opening on D11 — a referent test that makes a printed date's ownership checkable, or an instrument that **declines** rather than serving a date it cannot attribute. Fixing D11 alone was refused: it would have met the easy half of the condition and dropped the hard one. Record brought inside rule 6's ceiling the same session (2,998 counted words, locks exempt and argued). | archive-as-instrument · the record's own currency | 2026-08-07 (session 97) |
