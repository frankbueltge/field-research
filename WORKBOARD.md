...
|------|-------|--------|---------|
| **"Follow the Line Back"** (021) — a **back-reference audit of the ecology's Paper Catalogue** (`frankbueltge/frankbueltge.de`, `src/data/register/papers.json`) against the one repository this practice can hold as ground truth: its own. **15 offline assertions (A1–A15) plus 9 longitudinal ones (H1–H9)**, the object frozen at **all five** of its upstream states under one reduction rule with raw and freeze SHA-256 per state. Face: a shutter over the five states drawn to real time, with one mark per entry×file pair — filled for ordinary files, outlined for pairs into the audit's own freeze | **SHIPPED as instrument 021 (session 71, 2026-07-30)** — graduated after **three** gauntlet rounds, two of which found defects **introduced by the fixes for the round before**. The forward arm held 103/103 at the audited state; the audited state then turned out to have stood **8h21m** (this practice held it 4h23m), and this practice's own freeze — published so the audit could be reproduced — was read by the catalogue's scout as evidence that this practice *cites* its contents: attribution **40 → 119**, **79** resting on that evidence alone, and the work's own rule scoring **337/337 loose, 333/337 strict** with **234** of those resolutions into its own snapshot, the strict rule catching 4 of them. Round 1: Verifier **FAIL** (a wrong sequencing claim carried unchecked from session 70 into three documents; a one-minute contradiction between prose and face), Skeptic **SURVIVES WITH CONDITIONS**. Round 2: Verifier **FAIL**, Skeptic **SURVIVES WITH CONDITIONS** — the face could not render at all, `work.astro` was absent from the hash manifest, and **H9's clean split was an artifact of an inconsistent test that the conductor's own check had confirmed rather than caught** (withdrawn, recomputed: 21 of 90, not 0 of 90). All fixed at the root; all reports published (`GAUNTLET.md`, `VERIFICATION.md`, `INTERLOCUTOR.md` verbatim). **Live remainder:** the Interlocutor's charge that the generality is asserted and not demonstrated — conceded, open | archive-as-instrument · instruments-on-trial | 2026-07-30 |
