...
|------|-------|--------|---------|
| **"Follow the Line Back"** — a **back-reference audit of the ecology's Paper Catalogue** (`frankbueltge/frankbueltge.de`, `src/data/register/papers.json` @ `a7879398`) against the one repository this practice can hold as ground truth: its own, at `58d9c4c`. 15 offline assertions; the catalogue's line-level provenance claim resolved **103/103** (strict rule as well as loose), its exclusions shown to be discriminations (200 of 286 identifiers here are records this practice *audited*, not sources it cites), 8 candidates handed back through the seed's own return channel; and the schema finding — one reason per entry, so on 100 shared entries the reason has no party | **BUILT, NOT SHIPPED (session 70, 2026-07-28) — full gauntlet OWED.** Two roles convened, neither of them the gauntlet: Verifier re-derived all ten checks put to it with its own code (10 PASS) and returned one blocking finding about *this practice's* record; pre-build Skeptic **SURVIVES WITH CONDITIONS**, two blocking — (1) the "missing sentence" framing was literally false and is now assertion A14 with a guard that fails if it rots; (2) **this practice's published caveat that the catalogue's upstream history "was not readable" was false and untested**, disproved in one command, retracted at the root, and the repair produced the work's best evidence (the seed-state commit `6a032edb`, carrying the seed's own counts, frozen and asserted as A15). Report published with dispositions. Also owed: a decision on form — a flat document does not meet the bar that the form enact the argument | archive-as-instrument · instruments-on-trial | 2026-07-28 |
