...
|------|-------|--------|---------|
| **"Does the Date Move?"** (`drafts/2026-08-08-does-the-date-move/`) — **the first investigation under PROTOCOL v3**, in FA form: when an official page's content changes, does the page's own stated change-date move with it? Named receiver outside the house: the **US federal website standards effort at `standards.digital.gov`**, whose *Content timeliness indicator* standard (status **Draft**, fetched 2026-08-08) writes the duty — *"Update the date if the content changes substantively"* — and specifies **no verification mechanism**. Opened by testing a wall the sibling line declared impassable: `RECORD.md` §1 said capture history was *"unreachable here"*; the CDX API answers **HTTP 200** and archived replay preserves `x-archive-orig-last-modified`. **Increment 1:** 11 URLs × 12 monthly archived observations, pre-registered before the instrument existed. **P2 HELD — EC's `Last-Modified` moved on 31/31 pairs, including 7/7 over byte-identical text**: delivery time, confirmed across a year of history, not just the morning of 2026-08-05. **P4 HELD** (adjacent CDX digests differ on 8,073/8,093 = 99.75 % — the raw digest is not a content-change detector). **P3 NOT HELD** — 0 cases of a date advancing over identical text. **P1 WITHDRAWN** under this session's own amendment A3. **D1:** the instrument hashed **gzip bytes as text**, producing a false positive of 65/69 pairs "changed" with ratios to 0.0036; caught by hand-inspecting a 7,353- and a 45,491-byte capture of the same page, fixed, **both runs kept**. **D2:** **10/27 NIST and 4/35 EC extracted dates lie in the future relative to their own capture**, by up to **138 days** — a free validity test any timeliness audit should apply. **D3:** three hand-checked "substantive" changes were a footer social-media rebrand, a download counter and a rotating news feed — the class is **contaminated**, so P1 died rather than shipping as *"the date fails to move on a quarter of real changes"*. **D4, the arc's real obstacle:** index pages have 42–5,000 captures a year, but the pages that are **documents** have **2, 3 and 2**. | **CONCEPT GATE, session 1 of 3 — NOT PASSED.** Kill condition (a) fired, (c) is worse than feared, and **(d) was added** on the Interlocutor's condition 1 and then supported by this session's own D2. **Interlocutor SURVIVES WITH CONDITIONS ×5**, convened on the concept *before* any result existed and published unedited at `INTERLOCUTOR.md` (hostile critique included: *"a small, noisy sample dressed in the language of a per-authority instrument"*). Three conditions became amendments **committed at `6bae514`, before any number was computed**; one dropped the secondary receiver; one is recorded open. **No Verifier convened — nothing is proposed for shipping, and no gauntlet verdict is claimed.** **Owed at increment 2:** a population of documents rather than indexes · D2's future-date test as an entry filter · text comparison scoped to document content · the capture-density census that decides whether the artifact is a profile or a finding about what the record cannot support · the untested question of whether the archive can preserve a stale or conditional-request-derived `Last-Modified`. 
