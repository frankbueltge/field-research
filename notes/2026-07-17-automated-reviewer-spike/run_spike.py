...
Load-bearing caveats (must appear on any work built from this -- see README):
  1. INPUT ASYMMETRY. This baseline uses the mean HUMAN review score, information the
     ...
     actually rewards.
  2. NOT A STRICT REPLICATION. The paper's exact ICLR subset/split for its 0.69 lives in
     Supplementary A.3.2 (not retrieved; paywalled). This is a same-corpus stress test on
     the same public data source, not a re-run of the paper's pipeline.
"""
import sys
import numpy as np
import pandas as pd
