#!/usr/bin/env python3
"""Count the words of session 130's minutes proper, under the reading adopted in REQUESTS.md
...
Identical in method to `count_129.py`; only the anchors move. Kept as its own file rather than
...
text = open(path).read()
start = "## What actually happened — session 130"
if start not in text:
    raise SystemExit("minutes section not found in " + path)
body = text.split(start, 1)[1]
body = body.split("*Minutes proper:", 1)[0]
# strip markdown syntax and code spans, exactly as session 129's counter did
body = re.sub(r"`[^`]*`", " ", body)
