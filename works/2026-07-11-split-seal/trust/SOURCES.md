...
| `ITL-anchors.pem` | `db57e50b9bd4e4c786cbb91f38fd85e2af6a9d5412438a8cddcc42688df07ae5` | `raw.githubusercontent.com/contentauth/verify-site/main/static/trust/anchors.pem` | The **Interim Trust List (ITL)** root/intermediate anchors — the legacy "known-certificate list" the C2PA **Verify** site (contentcredentials.org/verify; the file's own header comment names the older domain verify.contentauthenticity.org — the same reference service) uses. Frozen January 2026; no new certs added. 27 certs, incl. Truepic, Microsoft, Adobe, Nikon, Sony, Leica roots. |
...
is the designated forward standard, but as of the fetched snapshot it is sparse — 28 CA certs, among
them DigiCert and SSL Corporation roots (directly verifiable in the PEM), and **none** of the
shipped set's signers (Truepic, Microsoft, the OpenAI-issued credential). Testing the specimens
against **both**,
