...
- Data-disclosure ranks: rank 464 (count 10), rank 891 (count 5), rank 1943 (count 2) in `provenance/envelope-pool.json` (4,432 types total) — matches README's stated ranks/counts. Confirmed these third-party names appear only inside the raw data file and nowhere in any shipped prose (README, work.astro, data.json, PREREGISTRATION.md, RESULTS-NOTE.md, meta.json) — full grep for AI/vendor names across all prose files returned nothing.
...
- Prohibited content: no AI product, company or tool vendor named as this practice's own tool anywhere in shipped prose; the only vendor-name occurrences are inside the raw, disclosed `provenance/envelope-pool.json` frequency table, consistent with the README's data-disclosure section.
...
- The two vendor names the Verifier quoted from the frequency table's tail are elided in this
  transcription (they are in the committed data file, which the README's data-disclosure section
  covers). That is the only alteration. An earlier version of this transcription also dropped the
