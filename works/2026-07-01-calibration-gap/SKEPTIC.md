...
| 2 | **Executed, then corrected.** The first execution over-extended this condition: it listed the suspension and the failing grade as uncorroborated, when the injunction order states both in the court's own summary. A closing micro-check by the same Verifier caught it, blocking. The note now names what the order does support (EMBA enrolment, French national on an investor visa, the F, the one-year suspension) and what nothing readable supports (papers by Yale scholars, a former university president). **Opening the order also produced a caveat about the case that this register had never applied** — per the order, the Honor Committee did not rely on the detector scans — now carried on the row at display prominence, as the Minnesota row has carried the equivalent since 2026-07-12. |
