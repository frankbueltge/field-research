...
188 of 521 works, and 187 of those 188 tripped our screen, while nothing else in the catalogue
provably did.
...
time — checked against a reader who was not allowed to see what the screen had decided.
...
## What came out — five of seven predictions refuted
...
association between the screen's flags and who supplied the record is overwhelming in all three
catalogues, at the floor of what the test can report. What does not travel is the concentration:
at home one source held 86.49 % of the flags; abroad the largest contributor holds 29.23 %
(Cleveland), 22.98 % (govdata.de) and 12.05 % (data.gov.uk) of them. **Open question 44 is answered, and the answer is: the relation travels, the singleness
does not.** The bar we set for concentration was also not scale-free, and we say so without moving
it — with 131 publishing organisations and more than half the values flagged, no single one can
hold twice its share.

**3. "Our four rules are really one rule" was a fact about our catalogue, not about the screen.**
At home the broad aggregate was the truncation rule alone on 249 of 252 held-out entries. Abroad
the rules separate: they agree on 48 %, 83 % and 72 %. The defect we published against ourselves
on 2026-09-08 was true, and we described it more broadly than the evidence allowed.
...
**The reason is structural.** Two of the five rules — a text repeated across records, and a
description that is its own title — are relations between a value and the rest of the catalogue. A
reader shown one value alone cannot see either. *Our pre-registered audit could not validate two of
its own rules, and we did not notice until the labels came back.* A second pass over the same
sixty values, declared after the fact, gave the reader one extra fact — how many records carry this
identical text — and agreement rose to 73 % and κ to 0.47. **The hollowness this screen finds is a
property of the catalogue, not of the value.**

**Two predictions survive, and both are about the instrument's own limits.** The English word list
...
partly a comparison of languages.* And the new title-echo rule earns its place: it flags 11.06 % of
German values, 1,204 of them caught by nothing else.

**The home arm reproduced to the digit** — 521 records, 40.5 %, 44.05 %, zero duplicate hits,
249 of 252 — so none of the above is the instrument having changed.

## The concrete thing, if you remember one

Cleveland's catalogue carries one careful paragraph about an illuminated manuscript, and attaches
it, unchanged, to **506** separate records — one for each leaf. A completeness metric counts 506
present values. A reader of any one of them learns about the book and nothing about the leaf. That
is defensible curation and invisible measurement at the same time, and naming it is the point;
grading it is not.
...
German open data landscape years ago, and the paper is quoted on the page. And nothing here says
anything about any institution's practice beyond what its own interface returned, quoted with the
record it came from.
